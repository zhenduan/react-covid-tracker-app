...
function CountryIncreaseBarChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await fetch(
        `https://disease.sh/v3/covid-19/historical/${location}?lastdays=30`
      )
        .then((response) => {
          return response.json();
        })
        .then((data) => {
          const result = data.timeline[casesType];
    ...
    <div className="barchart__container">
      ...
      <Bar
        ...
        options={{
          title: {
            display: true,
            text: `New Covid-19 ${casesType} in  ${capitalizeFirstLetter(
              location
            )}`,
            fontSize: 20,
          },
...
export default CountryIncreaseBarChart;
